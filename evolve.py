...
# ==========================================
# 0d. 读取历史"永久冻结"名单 —— 用于从候选池源头排除，而不只是写账时排除
# ==========================================
def get_frozen_tickers():
    """
    读取 trade_history.csv 全部历史记录（不限30天窗口），找出曾经被标记为
    Forced_Exit（突发利空强清）/ Trap_Warning（诱多陷阱）/
    Stop_Loss_Hit（止损触发）/ Period_Matured（持有到期）的所有 ticker。

    这份名单会在 get_top_300_pool() 之后立刻用来过滤候选池，
    让这些标的从源头上就不会进入 AI 看到的资金池——
    自然也就不会被 AI 重新推荐、不会出现在今日的 scan 邮件里。

    这是为了和"写账时的 FROZEN_TAGS 过滤"保持完全一致：
    避免出现"AI在报告里重新推荐了某只票，但写账时又被悄悄过滤掉、根本没被追踪"
    这种邮件内容和实际追踪状态不一致、容易误导用户去手动买入的情况。

    注意：这是按"该 ticker 历史上是否曾出现过冻结标签"判断，一旦冻结即永久排除，
    不会随时间自动解冻——如果某天你确认某只票值得重新考虑，
    需要手动去 trade_history.csv 里清掉/改掉它那一行的 Tag，才能让它重新进入候选池。
    """
    log_file = "trade_history.csv"
    ...
    try:
        ...
        if 'Tag' not in df.columns or 'Ticker' not in df.columns:
            return set()
        frozen = set(df.loc[df['Tag'].isin(FROZEN_TAGS), 'Ticker'].astype(str))
        if frozen:
            print(f"🧊 [阶段0d] 历史永久冻结名单共 {len(frozen)} 只，将在候选池阶段直接剔除: {frozen}")
        return frozen
    except Exception as e:
        print(f"⚠️ [阶段0d] 读取历史冻结名单失败: {e}")
        return set()
...
if __name__ == "__main__":
    ...
    # 阶段0d：把历史上曾被永久冻结（强清/止损/到期/陷阱预警）的标的从候选池里彻底剔除，
    # 让它们从源头上就不会被AI看到，自然也不会再出现在今日的scan邮件里——
    # 除非你手动去 trade_history.csv 清掉对应历史行，让它重新"解冻"进入候选池。
    if full_pool:
        frozen_tickers_all = get_frozen_tickers()
        if frozen_tickers_all:
            before_n = len(full_pool)
            full_pool = {t: v for t, v in full_pool.items() if t not in frozen_tickers_all}
            codes = [c for c in codes if c not in frozen_tickers_all]
            removed_n = before_n - len(full_pool)
            if removed_n > 0:
                print(f"🧹 [阶段0d] 候选池过滤：剔除 {removed_n} 只历史已冻结标的，本轮不再进入AI视野。")
