...
def _get_macro_news_time_tag(dt_obj):
    if dt_obj is None:
        return "[📑前日-低权重]", "20% → 5分"
    hours = (get_bj_time() - dt_obj).total_seconds() / 3600
    if hours <= 6:
...
def get_free_macro_news():
    ...
    sources = [
        ...
        ("CNBC Markets", "https://search.cnbc.com/rs/search/combinedcms/view.xml?partnerId=wrss01&id=10000664"),
        ("Google News Macro", "https://news.google.com/rss/search?q=China+US+markets+economy+inflation+tariff+when:3d&hl=zh-CN&gl=CN&ceid=CN:zh-Hans"),
    ]
    now = get_bj_time()
    entries = []
        ...
        try:
            ...
            for item in root.findall('.//item')[:50]:
                ...
                if title_el is None or not title_el.text:
                    continue
                dt_obj = _parse_rss_date(date_el.text if date_el is not None else '')
                if dt_obj is None:
                    continue
                age_hours = (now - dt_obj).total_seconds() / 3600
                if age_hours < -2 or age_hours > 72:
                    continue
                title = _normalise_event_text(title_el.text)
                tag, weight = _get_macro_news_time_tag(dt_obj)
                entries.append((dt_obj, source_name, title, tag, weight))
            print(f"   ✅ {source_name} 节点抓取成功")
        except Exception as e:
            print(f"   ⚠️ {source_name} 节点抓取失败: {str(e)[:140]}")
    if not entries:
        return "暂无实时新闻，请基于最新可用数据与产业链逻辑推演。"
    # 标题去重
    dedup = {}
    for row in entries:
        key = re.sub(r"\W+", "", row[2].lower())[:180]
        dedup[key] = row
    entries = sorted(dedup.values(), key=lambda x: x[0], reverse=True)
    grouped = {"今日最新": [], "今日": [], "昨日": [], "前日": []}
    for dt_obj, source_name, title, tag, weight in entries:
        age_hours = (now - dt_obj).total_seconds() / 3600
        bucket = "今日最新" if age_hours <= 6 else ("今日" if age_hours <= 24 else ("昨日" if age_hours <= 48 else "前日"))
        grouped[bucket].append(f"{tag} [{source_name}] {dt_obj.strftime('%m-%d %H:%M')} - {title}")
    out = []
    for bucket in ["今日最新", "今日", "昨日", "前日"]:
        if grouped[bucket]:
            out.append(f"\n【{bucket}】")
            out.extend(grouped[bucket][:25])
    result = "\n".join(out).strip()
    print(f"✅ 盘前新闻矩阵组装完毕，共 {len(entries)} 条，已按时效递减排序。")
    return result
